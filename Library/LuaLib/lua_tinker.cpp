...
static int eq_s64(lua_State *L) {
  lua_pushboolean(L, memcmp(lua_topointer(L, 1), lua_topointer(L, 2),
                            sizeof(long long)) == 0);
  return 1;
}
...
void lua_tinker::dobuffer(lua_State *L, const char *buff, size_t len,
                          const char *n, bool isReturnValue) {
  lua_pushcclosure(L, on_error, 0);
  int errfunc = lua_gettop(L);
...
template <>
::three_kingdoms::CME5File *lua_tinker::read<three_kingdoms::CME5File *>(
...
void lua_tinker::push(lua_State *L, std::string ret) {
  ...
  std::copy(ret.begin(), ret.end(),
            stdext::checked_array_iterator<char *>(buffer, ret.size() + 1));
  buffer[ret.size()] = '\0';
...
template <>
void lua_tinker::push(lua_State *L, std::wstring ret) {
  int length =
      WideCharToMultiByte(CP_UTF8, 0, ret.c_str(), -1, NULL, 0, NULL, NULL);
  char *buffer = new char[length + 1];
  WideCharToMultiByte(CP_UTF8, 0, ret.c_str(), -1, buffer, length, NULL, NULL);
  lua_pushstring(L, buffer);
  delete[] buffer;
}
