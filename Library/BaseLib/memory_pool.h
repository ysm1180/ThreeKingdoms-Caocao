...
class MemoryPoolManager {
 public:
  ...
  void RegisterMemoryPool(MemoryPoolBase *pool);
 ...
 private:
  std::set<MemoryPoolBase *> _poolManager;

  static std::once_flag s_onceFlag;
  static std::unique_ptr<MemoryPoolManager> s_sharedMemoryPoolManager;
};

template <typename T>
class CMemoryPool : public MemoryPoolBase {
 public:
  struct ArrayMemory {
    T *pointer;
    int size;
  ...
  static CMemoryPool<T> &GetInstance() {
    ...
    if (s_sharedMemoryPool == nullptr) {
      ...
      MemoryPoolManager::GetInstance().RegisterMemoryPool(s_sharedMemoryPool);
    }
