...
int CME5File::GetItemCount() { return _ReadInt(1); }

int CME5File::GetGroupCount() { return _ReadInt(5); }

int CME5File::GetGroupStartItemIndex(int groupIndex) {
  return _ReadInt(GROUP_HEADER_START_OFFSET + 4 +
                  groupIndex * GROUP_HEADER_SIZE);
}

int CME5File::GetGroupEndItemIndex(int groupIndex) {
  return _ReadInt(GROUP_HEADER_START_OFFSET + 8 +
                  groupIndex * GROUP_HEADER_SIZE);
}

int CME5File::GetOffset(int index) {
  int groupCount = GetGroupCount();
  return _ReadInt(index * HEADER_SIZE + groupCount * GROUP_HEADER_SIZE +
                  GROUP_HEADER_START_OFFSET);
}
...
size_t CME5File::GetGroupNameLength(int groupIndex) {
  return _ReadUnsignedInt(GROUP_HEADER_START_OFFSET +
                          groupIndex * GROUP_HEADER_SIZE);
}

size_t CME5File::GetNameLength(int index) {
  int groupCount = GetGroupCount();
  return _ReadUnsignedInt(index * HEADER_SIZE + groupCount * GROUP_HEADER_SIZE +
                          GROUP_HEADER_START_OFFSET + 4);
}
