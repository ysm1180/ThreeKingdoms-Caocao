...
LRESULT CRadioButtonControl::OnControlProc(HWND hWnd, UINT msg, WPARAM wParam,
                                           LPARAM lParam) {
  ...
  switch (msg) {
    case WM_CREATE: {
      ...
      auto radioButtonControl =
          reinterpret_cast<CRadioButtonControl *>(lpParamCreate);
      SetWindowLongPtr(hWnd, GWLP_USERDATA,
                       reinterpret_cast<LONG_PTR>(radioButtonControl));

      auto createEvent = radioButtonControl->GetCreateEvent();
      if (createEvent != LUA_NOREF) {
        LuaTinkerHelper::GetLuaTinker().Call(createEvent, radioButtonControl);
      }
    ...
    case WM_LBUTTONUP: {
      auto radioButtonControl = reinterpret_cast<CRadioButtonControl *>(
          GetWindowLongPtr(hWnd, GWLP_USERDATA));
      auto mouseLButtonUpEvent = radioButtonControl->GetMouseLButtonUpEvent();
      if (mouseLButtonUpEvent != LUA_NOREF) {
        LuaTinkerHelper::GetLuaTinker().Call(mouseLButtonUpEvent, radioButtonControl,
    ...
    case WM_MOUSEMOVE: {
      ...
      if (mouseMoveEvent != LUA_NOREF) {
        LuaTinkerHelper::GetLuaTinker().Call(mouseMoveEvent, radioButtonControl,
                                        (int)wParam, GET_X_LPARAM(lParam),
                                        GET_Y_LPARAM(lParam));
      }
    ...
    case WM_MOUSEHOVER: {
      ...
      if (mouseHoverEvent != LUA_NOREF) {
        LuaTinkerHelper::GetLuaTinker().Call(mouseHoverEvent, radioButtonControl);
      }
    ...
    case WM_MOUSELEAVE: {
      ...
      if (mouseLeaveEvent != LUA_NOREF) {
        LuaTinkerHelper::GetLuaTinker().Call(mouseLeaveEvent, radioButtonControl);
      }
    ...
    case WM_DESTROY: {
      ...
      if (destroyEvent != LUA_NOREF) {
        LuaTinkerHelper::GetLuaTinker().Call(destroyEvent, radioButtonControl);
      }
      break;
    }
  }
  return CallWindowProc(CRadioButtonControl::GetOriginalProc(), hWnd, msg,
                        wParam, lParam);
}

void CRadioButtonControl::RegisterFunctions(lua_State *L) {
  LUA_BEGIN_CHILD(CRadioButtonControl, "_RadioButtonControl",
                  CWindowChildControl);
...
CRadioButtonControl::CRadioButtonControl(bool isGroupStart) : _font(this) {
  _style = WS_TABSTOP | WS_CHILD | BS_AUTORADIOBUTTON |
           (isGroupStart ? WS_GROUP : 0);
  _type = L"radiobutton";
}
...
std::wstring CRadioButtonControl::GetText() { return _text; }

CTextFont *CRadioButtonControl::GetFont() { return &_font; }

HTHEME CRadioButtonControl::GetTheme() { return _theme; }
...
bool CRadioButtonControl::Create() {
  if (_parentControl != nullptr) {
    _hWnd = CreateWindow(L"jojo_radio", _text.c_str(), _style, _position.x,
...
WNDPROC CRadioButtonControl::GetOriginalProc() { return s_originalProc; }
}  // namespace three_kingdoms