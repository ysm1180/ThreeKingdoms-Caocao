...
int CToolbarManager::_GetNewIndex()
{
    ...
    {
        const auto index = static_cast<int>(TOOLBAR_INDEX::START_INDEX) + _toolbarButtonStorage.size();
...
int CToolbarManager::AddToolbarButton(CToolbarButton* item)
{
    ...
    _toolbarButtonStorage.insert(std::map<int, CToolbarButton *>::value_type(index, item));
    return index;
}
