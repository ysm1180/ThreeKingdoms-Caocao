...
int CLayoutControl::GetX() const { return _position.x; }

int CLayoutControl::GetY() const { return _position.y; }

int CLayoutControl::GetWidth() const { return _size.cx; }

int CLayoutControl::GetHeight() const { return _size.cy; }

bool CLayoutControl::IsHide() const { return _isHide; }
    ...
    if (isRedraw) {
      if (!_parents.empty()) {
        for (auto &parent : _parents) {
          ...
          RECT rect;
          SetRect(&rect, _position.x, _position.y,
                  _position.x + int(width * _ratioX),
...
void CLayoutControl::RemoveParentWIndow(CWindowControl *parent) {
  _parents.erase(std::remove(_parents.begin(), _parents.end(), parent),
                 _parents.end());
}
...
int CLayoutControl::AddImage(CImageControl *image, int x, int y, bool isShow) {
  ...
  GetDIBits(imageDC, image->GetImageHandle(), 0, 0, nullptr, &bitmapInfo,
            DIB_RGB_COLORS);
  auto pixels = new BYTE[bitmapInfo.bmiHeader.biSizeImage];
  GetDIBits(imageDC, image->GetImageHandle(), 0, image->GetHeight(), pixels,
            &bitmapInfo, DIB_RGB_COLORS);

  BITMAPINFOHEADER *bitmapInfoHeader = (BITMAPINFOHEADER *)&bitmapInfo;
  HBITMAP newBitmap = CreateDIBitmap(imageDC, bitmapInfoHeader, CBM_INIT,
  ...
  if (image->GetMirrorImageHandle() != nullptr) {
    ...
    HBITMAP oldMirrorBitmap =
        SelectBitmap(mirrorDC, image->GetMirrorImageHandle());
    auto bitmapInfo = image->GetBitmapInfo();
    GetDIBits(mirrorDC, image->GetMirrorImageHandle(), 0, 0, nullptr,
              &bitmapInfo, DIB_RGB_COLORS);
    auto mirrorPixels = new BYTE[bitmapInfo.bmiHeader.biSizeImage];
    GetDIBits(mirrorDC, image->GetMirrorImageHandle(), 0, image->GetHeight(),
              mirrorPixels, &bitmapInfo, DIB_RGB_COLORS);

    BITMAPINFOHEADER *bitmapInfoHeader = (BITMAPINFOHEADER *)&bitmapInfo;
    newMirrorBitmap = CreateDIBitmap(mirrorDC, bitmapInfoHeader, CBM_INIT,
                                     mirrorPixels, &bitmapInfo, DIB_RGB_COLORS);
  ...
  imageInfo.oldMirrorBitmap = newMirrorBitmap != nullptr
                                  ? SelectBitmap(newMirrorDC, newMirrorBitmap)
                                  : nullptr;
...
void CLayoutControl::MoveImage(int index, int x, int y, bool isUpdate) {
  ...
  while (iter != std::end(_images)) {
    if (iter->index == index) {
      ...
      SetRect(&rect, imageX, imageY,
              imageX + int(iter->image->GetWidth() * _ratioX),
    ...
    if (iter->index == index) {
      ...
      if (!_parents.empty()) {
        for (auto &parent : _parents) {
          ...
          SetRect(&rect, textX, textY,
                  textX + int(text->GetWidth(hdc) * _ratioX),
    ...
    if (iter->index == index) {
      if (!_parents.empty()) {
        for (auto &parent : _parents) {
          ...
          HDC hdc = GetDC(parent->GetHWnd());
          SetRect(&rect, textX, textY,
                  textX + int(iter->text->GetWidth(hdc) * _ratioX),
...
void CLayoutControl::Draw(HDC destDC) {
  if (!_isHide) {
    for (ImageInformation image : _images) {
      if (!image.isHide) {
        HDC imageDC =
            image.image->IsDisplayMirror() ? image.mirrorDC : image.imageDC;
        ...
        if (_ratioX == 1.0 && _ratioY == 1.0) {
          ...
          BitBlt(destDC, imageX, imageY, imageWidth, imageHeight, imageDC,
                 image.image->GetClipingLeft(), image.image->GetClipingTop(),
                 SRCINVERT);
          BitBlt(destDC, imageX, imageY, imageWidth, imageHeight, maskDC,
                 image.image->GetClipingLeft(), image.image->GetClipingTop(),
                 SRCAND);
          BitBlt(destDC, imageX, imageY, imageWidth, imageHeight, imageDC,
                 image.image->GetClipingLeft(), image.image->GetClipingTop(),
          ...
          auto memBitmap = CreateCompatibleBitmap(destDC, originalImageWidth,
                                                  originalImageHeight);
          auto oldBitmap = SelectBitmap(memDC, memBitmap);

          auto maskBitmap =
              CreateBitmap(imageWidth, imageHeight, 1, 1, nullptr);
          auto maskDC = CreateCompatibleDC(destDC);
          auto oldMask = SelectBitmap(maskDC, maskBitmap);

          SetStretchBltMode(memDC, COLORONCOLOR);
          StretchBlt(memDC, 0, 0, imageWidth, imageHeight, imageDC,
                     image.image->GetClipingLeft(),
                     image.image->GetClipingTop(),
                     image.image->GetClipingWidth(),
                     image.image->GetClipingHeight(), SRCCOPY);

          auto oldColor = SetBkColor(memDC, image.image->GetMaskColor());
          BitBlt(maskDC, 0, 0, originalImageWidth, originalImageHeight, memDC,
                 0, 0, SRCCOPY);
          ...
          BitBlt(destDC, imageX, imageY, imageWidth, imageHeight, memDC, 0, 0,
                 SRCINVERT);
          BitBlt(destDC, imageX, imageY, imageWidth, imageHeight, maskDC, 0, 0,
                 SRCAND);
          BitBlt(destDC, imageX, imageY, imageWidth, imageHeight, memDC, 0, 0,
                 SRCINVERT);
    ...
    for (RectInformation rect : _rects) {
      if (!rect.isHide) {
        ...
        Gdiplus::Rect rectangle(rect.position.x, rect.position.y,
                                rect.position.x + rect.rect->GetWidth(),
                                rect.position.y + rect.rect->GetHeight());
...
void CLayoutControl::Draw(HDC destDC, RECT &clipingRect) {
  if (!_isHide) {
    ...
    RECT layoutRect;
    SetRect(&layoutRect, _position.x, _position.y, _position.x + _size.cx,
            _position.y + _size.cy);
    ...
    for (ImageInformation image : _images) {
      if (!image.isHide) {
        ...
        if (_ratioX == 1.0 && _ratioY == 1.0) {
          ...
          RECT imageRect;
          RECT realDrawRect;
          SetRect(&imageRect, imageX, imageY, imageX + imageWidth,
                  imageY + imageHeight);
          ...
          BitBlt(
              destDC, realDrawRect.left, realDrawRect.top,
              realDrawRect.right - realDrawRect.left,
              realDrawRect.bottom - realDrawRect.top, imageDC,
              realDrawRect.left - imageRect.left +
                  image.image->GetClipingLeft(),
              realDrawRect.top - imageRect.top + image.image->GetClipingTop(),
              SRCINVERT);
          BitBlt(
              destDC, realDrawRect.left, realDrawRect.top,
              realDrawRect.right - realDrawRect.left,
          ...
          SetStretchBltMode(memDC, COLORONCOLOR);
          StretchBlt(memDC, 0, 0, originalImageWidth, originalImageHeight,
                     imageDC, image.image->GetClipingLeft(),
                     image.image->GetClipingTop(),
                     image.image->GetClipingWidth(),
                     image.image->GetClipingHeight(), SRCCOPY);
          ...
          BitBlt(destDC, realDrawRect.left, realDrawRect.top,
                 realDrawRect.right - realDrawRect.left,
                 realDrawRect.bottom - realDrawRect.top, memDC,
                 realDrawRect.left - imageRect.left,
    ...
    for (ImageInformation image : _images) {
      if (!image.isHide) {
        ...
        if (_ratioX == 1.0 && _ratioY == 1.0) {
          ...
          GetDIBits(imageDC, image.image->GetImageHandle(), 0, 0, nullptr,
                    &bitmapInfo, DIB_RGB_COLORS);

          int bits = bitmapInfo.bmiHeader.biBitCount / 8;
          auto pixels = new BYTE[bitmapInfo.bmiHeader.biSizeImage];
          GetDIBits(imageDC, image.image->GetImageHandle(), 0, height, pixels,
                    &bitmapInfo, DIB_RGB_COLORS);
          ...
          for (int y = height - drawingImageY - 1;
               y > height - drawingImageY - cy - 1; --y) {
            for (int x = drawingImageX; x < drawingImageX + cx; ++x) {
              ...
              RgbToHsv(GetRValue(mixedColor), GetGValue(mixedColor),
                       GetBValue(mixedColor), h2, s2, v2);
              h = h2;
              s = s2;
          ...
          HBITMAP newBitmap =
              CreateDIBitmap(imageDC, bitmapInfoHeader, CBM_INIT, pixels,
                             &bitmapInfo, DIB_RGB_COLORS);
  ...
  for (auto &parent : _parents) {
    for (ImageInformation image : tempImages) {
      if (!image.isHide) {
        ...
        RECT rect;
        SetRect(&rect, imageX, imageY,
                imageX + int(image.image->GetClipingWidth() * _ratioX),
    ...
    for (TextInformation text : tempTexts) {
      if (!text.isHide) {
        ...
        RECT rect;
        SetRect(&rect, textX, textY,
                textX + int(text.text->GetWidth(hdc) * _ratioX),
...
int CLayoutControl::AddRect(CGraphicRect *graphicRect, int x, int y,
                            bool isShow) {
  ...
  while (iter != std::end(_rects)) {
    if (iter->index == index) {
      ...
      SetRect(&rect, rectX, rectY,
              rectX + int(graphicRect->GetWidth() * _ratioX),
...
void CLayoutControl::MoveRect(int index, int x, int y, bool isUpdate) {
  ...
  while (iter != std::end(_rects)) {
    if (iter->index == index) {
      ...
      RECT rect;
      SetRect(&rect, rectX, rectY,
              rectX + int(iter->rect->GetWidth() * _ratioX),
    ...
    for (auto &parent : _parents) {
      ...
      for (RectInformation &rect : _rects) {
        if (rect.isRefresh) {
          ...
          RECT rc;
          SetRect(&rc, rectX, rectY,
                  rectX + int(rect.rect->GetWidth() * _ratioX),
