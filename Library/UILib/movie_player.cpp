...
void AllocPicture(void *userdata) {
  ...
  if (numBytes < 0) {
    Console::OutputConsoles(L"Cannot get image buffer size");
    return;
  }
...
bool CMoviePlayerControl::Create() {
  ...
  if (videoCodecParameters == nullptr || audioCodecParameters == nullptr) {
    Console::OutputConsoles(L"Cannot find video or audio stream");
    return false;
  }
  ...
  if (videoCodec == nullptr) {
    Console::OutputConsoles(L"Cannot find video decoder");
    return false;
  }
...
void CMoviePlayerControl::Play() {
  ...
  if (_endEvent != LUA_NOREF) {
    LuaTinkerHelper::GetLuaTinker().Call(_endEvent, this);
  }
