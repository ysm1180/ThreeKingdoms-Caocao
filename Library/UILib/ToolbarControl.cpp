...
void CToolbarControl::AddButton(CToolbarButton *button)
{
    ...
    if (index != static_cast<int>(TOOLBAR_INDEX::FULL_INDEX))
    {
        ...
        if (button->GetImage())
        {
            ...
            auto iter = std::find(_imageList.begin(), _imageList.end(), button->GetImage());
...
std::wstring CToolbarButton::GetText()
{
    return _text;
}
...
std::wstring CToolbarButton::GetTooltipText()
{
    return _tooltipText;
}
...
void CToolbarButton::SetIndex(int index)
{
    _button.idCommand = index;
}
