...
LRESULT CComboBoxControl::OnControlProc(HWND hWnd, UINT msg, WPARAM wParam,
                                        LPARAM lParam) {
  ...
  switch (msg) {
    case WM_CREATE: {
      ...
      auto comboBoxControl =
          reinterpret_cast<CComboBoxControl *>(lpParamCreate);
      SetWindowLongPtr(hWnd, GWLP_USERDATA,
                       reinterpret_cast<LONG_PTR>(comboBoxControl));

      auto createEvent = comboBoxControl->GetCreateEvent();
      if (createEvent != LUA_NOREF) {
        LuaTinkerHelper::GetLuaTinker().Call(createEvent, comboBoxControl);
      }
      break;
    }

    case WM_LBUTTONUP: {
      auto comboBoxControl = reinterpret_cast<CComboBoxControl *>(
          GetWindowLongPtr(hWnd, GWLP_USERDATA));
      auto mouseLButtonUpEvent = comboBoxControl->GetMouseLButtonUpEvent();
      if (mouseLButtonUpEvent != LUA_NOREF) {
        LuaTinkerHelper::GetLuaTinker().Call(mouseLButtonUpEvent, comboBoxControl,
    ...
    case WM_MOUSEMOVE: {
      ...
      if (mouseMoveEvent != LUA_NOREF) {
        LuaTinkerHelper::GetLuaTinker().Call(mouseMoveEvent, comboBoxControl,
                                        (int)wParam, GET_X_LPARAM(lParam),
                                        GET_Y_LPARAM(lParam));
      }
    ...
    case WM_MOUSEHOVER: {
      ...
      if (mouseHoverEvent != LUA_NOREF) {
        LuaTinkerHelper::GetLuaTinker().Call(mouseHoverEvent, comboBoxControl);
      }
    ...
    case WM_MOUSELEAVE: {
      ...
      if (mouseLeaveEvent != LUA_NOREF) {
        LuaTinkerHelper::GetLuaTinker().Call(mouseLeaveEvent, comboBoxControl);
      }
    ...
    case WM_DESTROY: {
      ...
      if (destroyEvent != LUA_NOREF) {
        LuaTinkerHelper::GetLuaTinker().Call(destroyEvent, comboBoxControl);
      }
      break;
    }
  }
  return CallWindowProc(CComboBoxControl::GetOriginalProc(), hWnd, msg, wParam,
                        lParam);
}
...
CComboBoxControl::CComboBoxControl() : _font(this) {
...
bool CComboBoxControl::Create() {
  if (_parentControl != nullptr) {
    _hWnd =
        CreateWindow(L"jojo_combo", L"", _style, _position.x, _position.y,
...
WNDPROC CComboBoxControl::GetOriginalProc() { return s_originalProc; }
}  // namespace three_kingdoms