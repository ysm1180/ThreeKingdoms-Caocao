...
LRESULT CStaticControl::OnControlProc(HWND hWnd, UINT msg, WPARAM wParam,
                                      LPARAM lParam) {
  switch (msg) {
    case WM_CREATE: {
      ...
      SetWindowLongPtr(hWnd, GWLP_USERDATA,
                       reinterpret_cast<LONG_PTR>(staticControl));

      auto createEvent = staticControl->GetCreateEvent();
      if (createEvent != LUA_NOREF) {
        LuaTinkerHelper::GetLuaTinker().Call(createEvent, staticControl);
      }
      break;
    }
    ...
    case WM_DESTROY: {
      ...
      if (destroyEvent != LUA_NOREF) {
        LuaTinkerHelper::GetLuaTinker().Call(destroyEvent, staticControl);
      }
      break;
    }
  }

  return CallWindowProc(CStaticControl::GetOriginalProc(), hWnd, msg, wParam,
                        lParam);
}

void SplitString(std::wstring src, std::wstring delimiter,
...
CStaticControl::CStaticControl() : _font(this) {
  _type = L"static";
...
bool CStaticControl::IsAutoSize() { return _isAutoSize; }

bool CStaticControl::IsTransparentBackground() {
  return _isTransparentBackground;
}

int CStaticControl::GetAlign() { return _align; }

std::wstring CStaticControl::GetText() { return _text; }

CTextFont *CStaticControl::GetFont() { return &_font; }

COLORREF CStaticControl::GetTextColor() { return _textColor; }

HBRUSH CStaticControl::GetBackgroundBrush() { return _backgroundBrush; }
...
bool CStaticControl::Create() {
  if (_parentControl != nullptr) {
    _hWnd = CreateWindow(L"jojo_static", _text.c_str(), _style, _position.x,
...
WNDPROC CStaticControl::GetOriginalProc() { return s_originalProc; }
}  // namespace three_kingdoms