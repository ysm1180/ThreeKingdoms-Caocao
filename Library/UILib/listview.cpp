...
LRESULT CListViewControl::OnControlProc(HWND hWnd, UINT msg, WPARAM wParam,
                                        LPARAM lParam) {
  ...
  switch (msg) {
    case WM_CREATE: {
      ...
      SetWindowLongPtr(hWnd, GWLP_USERDATA,
                       reinterpret_cast<LONG_PTR>(listView));

      auto createEvent = listView->GetCreateEvent();
      if (createEvent != LUA_NOREF) {
        LuaTinkerHelper::GetLuaTinker().Call(createEvent, listView);
      }
      break;
    }
    ...
    case WM_LBUTTONUP: {
      ...
      if (mouseLButtonUpEvent != LUA_NOREF) {
        LuaTinkerHelper::GetLuaTinker().Call(
            mouseLButtonUpEvent, listView, static_cast<int>(wParam),
    ...
    case WM_MOUSEMOVE: {
      ...
      if (mouseMoveEvent != LUA_NOREF) {
        LuaTinkerHelper::GetLuaTinker().Call(
            mouseMoveEvent, listView, static_cast<int>(wParam),
            GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
      }
    ...
    case WM_MOUSEHOVER: {
      ...
      if (!listView->_isHover) {
        ...
        if (mouseEnterEvent != LUA_NOREF) {
          LuaTinkerHelper::GetLuaTinker().Call(mouseEnterEvent, listView);
        }
      }
    ...
    case WM_MOUSELEAVE: {
      ...
      if (mouseLeaveEvent != LUA_NOREF) {
        LuaTinkerHelper::GetLuaTinker().Call(mouseLeaveEvent, listView);
      }
    ...
    case WM_DESTROY: {
      ...
      if (destroyEvent != LUA_NOREF) {
        LuaTinkerHelper::GetLuaTinker().Call(destroyEvent, listView);
      }
  ...
  return CallWindowProc(CListViewControl::GetOriginalProc(), hWnd, msg, wParam,
                        lParam);
}
...
bool CListViewColumn::IsAutoSizeFitItem() const { return _isAutoSizeFitItem; }

bool CListViewColumn::IsAutoSizeFitHeader() const {
  return _isAutoSizeFitHeader;
}

int CListViewColumn::GetIndex() const { return _index; }

int CListViewColumn::GetAlign() const { return _align; }
...
std::wstring CListViewColumn::GetText() const { return _text; }
...
void CListViewColumn::SetAutoSizeFitHeader(bool value) {
  ...
  if (_parentListView) {
    ListView_SetColumnWidth(_parentListView->GetHWnd(), _index,
                            LVSCW_AUTOSIZE_USEHEADER);
  }
}

void CListViewColumn::SetIndex(int index) { _index = index; }
...
std::wstring CListViewItem::GetText() { return _text; }

CTextFont *CListViewItem::GetFont() { return &_font; }
...
COLORREF CListViewItem::GetNormalTextColor() { return _textColor.normal; }

COLORREF CListViewItem::GetFocusedTextColor() { return _textColor.focused; }

COLORREF CListViewItem::GetDisabledTextColor() { return _textColor.disabled; }

COLORREF CListViewItem::GetDisableFocusedTextColor() {
  return _textColor.disableFocused;
}

int CListViewItem::GetAlign() { return _align; }

void CListViewItem::SetParentListRow(CListViewRow *parent) {
  _parentListRow = parent;
}

void CListViewItem::SetText(std::wstring text) { _text = text; }
...
void CListViewItem::SetItemIndex(int itemIndex) { _itemIndex = itemIndex; }
...
void CListViewItem::_Update() {
  if (_parentListRow && _parentListRow->GetParentListView()) {
    ...
    RECT rect;
    ListView_GetSubItemRect(listViewHWnd, _parentListRow->GetRowIndex(),
                            _itemIndex, LVIR_BOUNDS, &rect);
...
CListViewRow::~CListViewRow() {
  if (_activeEvent != LUA_NOREF) {
    luaL_unref(LuaTinkerHelper::GetLuaTinker().GetLuaState(), LUA_REGISTRYINDEX,
               _activeEvent);
  }
}

bool CListViewRow::IsEnabled() { return _isEnabled; }

CListViewControl *CListViewRow::GetParentListView() { return _parentListView; }

int CListViewRow::GetActiveEvent() { return _activeEvent; }
...
int CListViewRow::GetRowIndex() { return _rowIndex; }
...
void CListViewRow::SetEnabled(bool isEnabled) {
  ...
  if (_parentListView) {
    RECT rect;
    ListView_GetItemRect(_parentListView->GetHWnd(), _rowIndex, &rect,
                         LVIR_BOUNDS);
...
void CListViewRow::SetRowIndex(int index) { _rowIndex = index; }
...
bool CListViewControl::IsShowBorder() const { return _isShowBorder; }

bool CListViewControl::IsShowColumn() const { return _isShowColumn; }
...
bool CListViewControl::IsTrackingSelect() const { return _isTrackingSelect; }
...
int CListViewControl::GetColumnCount() { return _columns.size(); }

COLORREF CListViewControl::GetBackgroundColor() { return _backgroundColor; }

HBRUSH CListViewControl::GetBackgroundBrush() { return _backgroundBrush; }

int CListViewControl::GetRowHeight() { return _rowHeight; }
  ...
  if (_hWnd != nullptr) {
    ...
    for (auto i = 0; i < _columns.size(); ++i) {
      ...
      if (i == 0) {
        ...
        ListView_SetItemText(_hWnd, itemIndex, i,
...
bool CListViewControl::Create() {
  _hWnd =
      CreateWindow(L"jojo_listview", L"", _style, _position.x, _position.y,
...
WNDPROC CListViewControl::GetOriginalProc() { return s_originalProc; }
}  // namespace three_kingdoms