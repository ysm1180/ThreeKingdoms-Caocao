...
HFONT CTextFont::GetHFont() const { return _font; }

bool CTextFont::IsBold() const { return _isBold; }

bool CTextFont::IsItalic() const { return _isItalic; }

bool CTextFont::IsUnderline() const { return _isUnderline; }

int CTextFont::GetFontSize() const { return _fontSize; }

std::wstring CTextFont::GetFontName() const { return _fontName; }
...
void CTextFont::ResetFont() {
  ...
  _font = CreateFontW(
      _fontSize, 0, 0, 0, FW_NORMAL, static_cast<DWORD>(_isItalic),
      static_cast<DWORD>(_isUnderline), static_cast<DWORD>(_isBold),
      HANGEUL_CHARSET, 0, 0, 0, VARIABLE_PITCH | FF_ROMAN, _fontName.c_str());
  ...
  if (_control) {
    ...
    if (controlHWnd != nullptr) {
      ...
      RECT rect;
      SetRect(&rect, _control->GetX(), _control->GetY(),
              _control->GetX() + _control->GetWidth(),
