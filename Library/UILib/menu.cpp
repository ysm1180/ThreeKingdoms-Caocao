...
CMenuItem::CMenuItem() : _font() { _childMenu = nullptr; }
...
bool CMenuItem::IsEnabled() const { return _isEnabled; }

int CMenuItem::GetIndex() const { return _index; }

CMenu *CMenuItem::GetChildMenu() const { return _childMenu; }

std::wstring CMenuItem::GetText() const { return _text; }

int CMenuItem::GetClickEvent() const { return _clickEvent; }

CTextFont *CMenuItem::GetFont() { return &_font; }

MenuItemStateColor CMenuItem::GetBackgroundColor() const {
  return _backgroundColor;
}

MenuItemStateColor CMenuItem::GetTextColor() const { return _textColor; }
...
void CMenuItem::SetEnabled(bool isEnabled) {
  ...
  if (_parentMenu) {
    ...
    ::SetMenuItemInfo(_parentMenu->GetHMenu(), static_cast<UINT>(_index), FALSE,
                      &itemInfo);
...
void CMenuItem::SetText(std::wstring text) {
  ...
  if (_parentMenu) {
    ...
    ::SetMenuItemInfo(_parentMenu->GetHMenu(), static_cast<UINT>(_index), TRUE,
                      &itemInfo);
...
void CMenuItem::SetChildMenu(CMenu *childMenu) { _childMenu = childMenu; }

void CMenuItem::SetParentMenu(CMenu *parentMenu) { _parentMenu = parentMenu; }

void CMenuItem::SetIndex(int index) { _index = index; }
...
COLORREF CMenuItem::GetNormalTextColor() { return _textColor.normal; }

COLORREF CMenuItem::GetFocusedTextColor() { return _textColor.focused; }

COLORREF CMenuItem::GetDisabledTextColor() { return _textColor.disabled; }
...
HMENU CMenu::GetHMenu() { return _menu; }

CWindowControl *CMenu::GetParentWindow() { return _parentWindow; }
...
void CMenu::SetParentWindow(CWindowControl *parent) { _parentWindow = parent; }
...
void CMenu::AddMenuItem(CMenuItem *menuItem) {
  ...
  if (iter != _menuItems.end()) {
    Console::OutputConsoles(L"Already added menu item");
  } else {
    if (menuItem->GetChildMenu()) {
      int index = CMenuManager::GetInstance().AddMenuItemByHandle(
          menuItem, menuItem->GetChildMenu()->GetHMenu());
      AppendMenu(_menu, MF_STRING | MF_POPUP,
                 (UINT)menuItem->GetChildMenu()->GetHMenu(),
                 menuItem->GetText().c_str());
    } else {
      int index = CMenuManager::GetInstance().AddMenuItem(menuItem);
      if (menuItem->IsEnabled() &&
          wcscmp(menuItem->GetText().c_str(), L"-") != 0) {
