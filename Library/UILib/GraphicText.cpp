...
CTextFont *CGraphicText::GetFont() { return &_font; }

std::wstring CGraphicText::GetText() { return _text; }

COLORREF CGraphicText::GetTextColor() { return _textColor; }
...
void CGraphicText::SetText(std::wstring text) { _text = text; }

void CGraphicText::SetTextColor(COLORREF color) { _textColor = color; }
...
void CGraphicText::Draw(HDC hdc, POINT position) {
  ...
  RECT rect{0, 0, 0, 0};
  DrawText(hdc, _text.c_str(), _text.length(), &rect, DT_CALCRECT);
  SetRect(&rect, position.x, position.y, position.x + rect.right,
