...
LRESULT CALLBACK CWindowControl::OnControlProc(HWND hWnd, UINT iMessage,
  ...
  switch (iMessage) {
    case WM_CREATE: {
      ...
      // GetWindowLongPtr 을 통해 WindowControl instance 를 불러와서 함수 호출
      // 가능
      ...
      if (createEvent != LUA_NOREF) {
        LuaTinkerHelper::GetLuaTinker().Call(createEvent, window);
      }
    ...
    case WM_KEYDOWN: {
      auto window = reinterpret_cast<CWindowControl *>(
          GetWindowLongPtr(hWnd, GWLP_USERDATA));
      auto keyDownEvent = window->GetKeyDownEvent();
      if (keyDownEvent != LUA_NOREF) {
        LuaTinkerHelper::GetLuaTinker().Call(keyDownEvent, window, (int)wParam);
      }
      break;
    }
    ...
    case WM_KEYUP: {
      ...
      if (keyUpEvent != LUA_NOREF) {
        LuaTinkerHelper::GetLuaTinker().Call(keyUpEvent, window, (int)wParam);
      }
      break;
    }
    ...
    case WM_LBUTTONUP: {
      ...
      if (mouseLButtonUpEvent != LUA_NOREF) {
        ...
        LuaTinkerHelper::GetLuaTinker().Call(mouseLButtonUpEvent, window,
                                        (int)wParam, GET_X_LPARAM(lParam), y);
      }
      break;
    }
    ...
    case WM_LBUTTONDOWN: {
      ...
      if (mouseLButtonDownEvent != LUA_NOREF) {
        ...
        LuaTinkerHelper::GetLuaTinker().Call(mouseLButtonDownEvent, window,
                                        (int)wParam, GET_X_LPARAM(lParam), y);
      }
      break;
    }
    ...
    case WM_MOUSEMOVE: {
      ...
      if (mouseMoveEvent != LUA_NOREF) {
        ...
        LuaTinkerHelper::GetLuaTinker().Call(mouseMoveEvent, window, (int)wParam,
                                        GET_X_LPARAM(lParam), y);
      }
    ...
    case WM_MOUSEHOVER: {
      ...
      if (mouseHoverEvent != LUA_NOREF) {
        LuaTinkerHelper::GetLuaTinker().Call(mouseHoverEvent, window);
      }
    ...
    case WM_MOUSELEAVE: {
      ...
      if (mouseLeaveEvent != LUA_NOREF) {
        LuaTinkerHelper::GetLuaTinker().Call(mouseLeaveEvent, window);
      }
    ...
    case WM_ACTIVATEAPP: {
      if (wParam == TRUE) {
        ...
        if (activeEvent != LUA_NOREF) {
          LuaTinkerHelper::GetLuaTinker().Call(activeEvent, window);
        }
      }
      return 0;
    }
    ...
    case WM_COMMAND: {
      ...
      if (menuItem) {
        ...
        if (clickEvent != LUA_NOREF) {
          LuaTinkerHelper::GetLuaTinker().Call(clickEvent, menuItem);
        }
      }
    ...
    case WM_SIZE: {
      ...
      if (sizeEvent != LUA_NOREF) {
        LuaTinkerHelper::GetLuaTinker().Call(sizeEvent, window, width, height);
      }
      break;
    }
    ...
    case WM_CLOSE: {
      ...
      if (closeEvent != LUA_NOREF) {
        ...
        const auto notClose =
            LuaTinkerHelper::GetLuaTinker().Call<bool>(closeEvent, window);
        if (notClose) {
          return 0;
        }
      }
    ...
    case WM_DESTROY: {
      ...
      if (destroyEvent != LUA_NOREF) {
        LuaTinkerHelper::GetLuaTinker().Call<void>(destroyEvent, window);
      }
    ...
    case WM_NOTIFY: {
      ...
      if (pnmhdr->code == TTN_NEEDTEXT) {
        ...
        auto listView = reinterpret_cast<CListViewControl *>(
            GetWindowLongPtr(pnmhdr->hwndFrom, GWLP_USERDATA));
        ...
        if (listView) {
          ...
          if (row->IsEnabled()) {
            ...
            if (itemActiveEvent != LUA_NOREF) {
              LuaTinkerHelper::GetLuaTinker().Call<void>(
                  itemActiveEvent, listView, static_cast<int>(lpnmia->iItem),
    ...
    case WM_MEASUREITEM: {
      ...
      if (item->CtlType == ODT_MENU) {
        ...
        GetTextExtentPoint32(hdc, menuItem->GetText().c_str(),
                             menuItem->GetText().length(), &size);
    ...
    case WM_DRAWITEM: {
      ...
      if (item->CtlType == ODT_BUTTON) {
        auto button = reinterpret_cast<CButtonControl *>(
            GetWindowLongPtr(item->hwndItem, GWLP_USERDATA));
        ...
        RECT rect;
        SetRect(&rect, item->rcItem.left, item->rcItem.top, item->rcItem.right,
                item->rcItem.bottom);
        ...
        if (!button->IsTransparentBackground()) {
          ...
          SetRect(&rect, rect.left + borderWidth, rect.top + borderWidth,
        ...
        auto originalFont =
            SelectFont(item->hDC, button->GetFont()->GetHFont());
        ...
        if (wcscmp(menu->GetText().c_str(), L"-") == 0) {
          ...
          SetRect(&rect, item->rcItem.left + 4, item->rcItem.top,
          ...
          auto originalFont =
              SelectFont(item->hDC, menu->GetFont()->GetHFont());
          SetBkMode(item->hDC, TRANSPARENT);
        ...
        for (int i = 0; i < listview->GetColumnCount(); ++i) {
          ...
          if (row->IsEnabled()) {
            ...
            if (item->itemState & ODS_SELECTED) {
              backgroundColor =
                  listViewItem->GetDisableFocusedBackgroundColor();
              SetTextColor(item->hDC,
                           listViewItem->GetDisableFocusedTextColor());
              isSelected = true;
            } else {
          ...
          SetRect(&rect, left, item->rcItem.top, left + columnWidth,
                  item->rcItem.bottom);
          ...
          if (listview->IsTransparentBackground()) {
            ...
            RECT rectByWindow;
            SetRect(&rectByWindow, rect.left, rect.top, rect.right,
                    rect.bottom);
            ClientToScreen(listview->GetHWnd(),
                           reinterpret_cast<POINT *>(
                               &rectByWindow.left));  // convert top-left
            ClientToScreen(listview->GetHWnd(),
                           reinterpret_cast<POINT *>(
                               &rectByWindow.right));  // convert bottom-right
            ScreenToClient(hWnd, reinterpret_cast<POINT *>(&rectByWindow.left));
            ScreenToClient(hWnd,
                           reinterpret_cast<POINT *>(&rectByWindow.right));
          ...
          auto originalFont =
              SelectFont(item->hDC, listViewItem->GetFont()->GetHFont());
          SetBkMode(item->hDC, TRANSPARENT);
        ...
        auto originalFont =
            SelectFont(item->hDC, staticControl->GetFont()->GetHFont());
        SetBkMode(item->hDC, TRANSPARENT);
        if (staticControl->GetAlign() == 0) {
          DrawText(item->hDC, staticControl->GetText().c_str(), -1, &rect,
                   DT_LEFT | DT_WORD_ELLIPSIS);
        } else if (staticControl->GetAlign() == 1) {
          DrawText(item->hDC, staticControl->GetText().c_str(), -1, &rect,
                   DT_RIGHT | DT_WORD_ELLIPSIS);
    ...
    case WM_CTLCOLORSTATIC: {
      ...
      if (control->GetType() == L"static") {
        ...
        auto originalFont =
            SelectFont((HDC)wParam, groupBox->GetFont()->GetHFont());
        GetTextExtentPoint32((HDC)wParam, groupBox->GetText().c_str(),
                             groupBox->GetText().length(), &size);

        GetClientRect(groupBox->GetHWnd(), &rect);
        SetRect(&rect, rect.left + 4, rect.top, rect.left + size.cx + 4,
                rect.top + size.cy);
        SetBkMode((HDC)wParam, TRANSPARENT);
        DrawText((HDC)wParam, groupBox->GetText().c_str(), -1, &rect,
                 DT_CENTER | DT_VCENTER | DT_SINGLELINE);
        SelectFont((HDC)wParam, originalFont);
        SetBkMode((HDC)wParam, OPAQUE);

        ExcludeClipRect((HDC)wParam, rect.left, rect.top, rect.right,
                        rect.bottom + 2);
      } else if (control->GetType() == L"checkbox") {
        auto checkBox = reinterpret_cast<CCheckboxControl *>(
            GetWindowLongPtr((HWND)lParam, GWLP_USERDATA));
        ...
        int checkBoxHeight =
            GetSystemMetrics(SM_CYMENUCHECK) - GetSystemMetrics(SM_CYEDGE);
        ...
        RECT textRect;
        SetRect(&textRect, rect.left + checkBoxWidth + 4,
                rect.top + (height - 1) / 2 - checkBoxHeight / 2, rect.right,
                rect.bottom - (height / 2 - checkBoxHeight / 2) + 1);
        auto originalFont =
            SelectFont((HDC)wParam, checkBox->GetFont()->GetHFont());
        SetBkMode((HDC)wParam, TRANSPARENT);
        DrawText((HDC)wParam, checkBox->GetText().c_str(), -1, &textRect,
                 DT_LEFT | DT_VCENTER | DT_SINGLELINE);
        SelectFont((HDC)wParam, originalFont);
        SetBkMode((HDC)wParam, OPAQUE);

        RECT checkBoxRect;
        SetRect(&checkBoxRect, rect.left,
                rect.top + height / 2 - checkBoxHeight / 2,
                rect.left + checkBoxWidth,
                rect.bottom - (height / 2 - checkBoxHeight / 2));
        auto theme = checkBox->GetTheme();
        if (checkBox->IsChecked()) {
          if (checkBox->IsPressed()) {
            DrawThemeBackground(theme, (HDC)wParam, BP_CHECKBOX,
                                CBS_CHECKEDPRESSED, &checkBoxRect, nullptr);
          } else if (checkBox->IsHover()) {
            DrawThemeBackground(theme, (HDC)wParam, BP_CHECKBOX, CBS_CHECKEDHOT,
                                &checkBoxRect, nullptr);
          } else {
            if (checkBox->IsEnabled()) {
              DrawThemeBackground(theme, (HDC)wParam, BP_CHECKBOX,
                                  CBS_CHECKEDNORMAL, &checkBoxRect, nullptr);
            } else {
              DrawThemeBackground(theme, (HDC)wParam, BP_CHECKBOX,
                                  CBS_CHECKEDDISABLED, &checkBoxRect, nullptr);
            }
          }
        } else {
          if (checkBox->IsPressed()) {
            DrawThemeBackground(theme, (HDC)wParam, BP_CHECKBOX,
                                CBS_UNCHECKEDPRESSED, &checkBoxRect, nullptr);
          } else if (checkBox->IsHover()) {
            DrawThemeBackground(theme, (HDC)wParam, BP_CHECKBOX,
                                CBS_UNCHECKEDHOT, &checkBoxRect, nullptr);
          } else {
            if (checkBox->IsEnabled()) {
              DrawThemeBackground(theme, (HDC)wParam, BP_CHECKBOX,
                                  CBS_UNCHECKEDNORMAL, &checkBoxRect, nullptr);
            } else {
              DrawThemeBackground(theme, (HDC)wParam, BP_CHECKBOX,
                                  CBS_UNCHECKEDDISABLED, &checkBoxRect,
                                  nullptr);
            }
          }
        }
        ...
        auto radioButton = reinterpret_cast<CRadioButtonControl *>(
            GetWindowLongPtr((HWND)lParam, GWLP_USERDATA));
        ...
        RECT textRect;
        SetRect(&textRect, rect.left + radioWidth + 4,
                rect.top + (height - 1) / 2 - radioHeight / 2, rect.right,
                rect.bottom - (height / 2 - radioHeight / 2) + 1);
        auto originalFont =
            SelectFont((HDC)wParam, radioButton->GetFont()->GetHFont());
        SetBkMode((HDC)wParam, TRANSPARENT);
        DrawText((HDC)wParam, radioButton->GetText().c_str(), -1, &textRect,
                 DT_LEFT | DT_VCENTER | DT_SINGLELINE);
        SelectFont((HDC)wParam, originalFont);
        SetBkMode((HDC)wParam, OPAQUE);

        RECT radioButtonRect;
        SetRect(&radioButtonRect, rect.left,
                rect.top + height / 2 - radioHeight / 2, rect.left + radioWidth,
                rect.bottom - (height / 2 - radioHeight / 2));
        auto theme = radioButton->GetTheme();
        if (radioButton->IsChecked()) {
          if (radioButton->IsPressed()) {
            DrawThemeBackground(theme, (HDC)wParam, BP_RADIOBUTTON,
                                RBS_CHECKEDPRESSED, &radioButtonRect, nullptr);
          } else if (radioButton->IsHover()) {
            DrawThemeBackground(theme, (HDC)wParam, BP_RADIOBUTTON,
                                RBS_CHECKEDHOT, &radioButtonRect, nullptr);
          } else {
            if (radioButton->IsEnabled()) {
              DrawThemeBackground(theme, (HDC)wParam, BP_RADIOBUTTON,
                                  RBS_CHECKEDNORMAL, &radioButtonRect, nullptr);
            } else {
              DrawThemeBackground(theme, (HDC)wParam, BP_RADIOBUTTON,
                                  RBS_CHECKEDDISABLED, &radioButtonRect,
                                  nullptr);
            }
          }
        } else {
          if (radioButton->IsPressed()) {
            DrawThemeBackground(theme, (HDC)wParam, BP_RADIOBUTTON,
                                RBS_UNCHECKEDPRESSED, &radioButtonRect,
                                nullptr);
          } else if (radioButton->IsHover()) {
            DrawThemeBackground(theme, (HDC)wParam, BP_RADIOBUTTON,
                                RBS_UNCHECKEDHOT, &radioButtonRect, nullptr);
          } else {
            if (radioButton->IsEnabled()) {
              DrawThemeBackground(theme, (HDC)wParam, BP_RADIOBUTTON,
                                  RBS_UNCHECKEDNORMAL, &radioButtonRect,
                                  nullptr);
            } else {
              DrawThemeBackground(theme, (HDC)wParam, BP_RADIOBUTTON,
                                  RBS_UNCHECKEDDISABLED, &radioButtonRect,
                                  nullptr);
            }
          }
        }
    ...
    case WM_CTLCOLORBTN: {
      ...
      if (button->GetType() == L"button") {
        ...
        if (button->IsTransparentBackground()) {
          ...
          auto originalFont =
              SelectFont((HDC)wParam, button->GetFont()->GetHFont());
          SetBkMode((HDC)wParam, TRANSPARENT);
          DrawText((HDC)wParam, button->GetText().c_str(), -1, &rect,
                   DT_CENTER | DT_VCENTER | DT_SINGLELINE);
          ...
          int borderWidth = button->GetBorderWidth();
          ExcludeClipRect((HDC)wParam, rect.left + borderWidth,
                          rect.top + borderWidth, rect.right - borderWidth,
    ...
    case WM_PAINT: {
      ...
      HBITMAP bitmap =
          CreateCompatibleBitmap(hdc, window->GetWidth(), window->GetHeight());
      ...
      BitBlt(hdc, 0, 0, window->GetWidth(), window->GetHeight(), memDC, 0, 0,
             SRCCOPY);
...
bool CWindowControl::IsControlBox() const { return _isControlBox; }

bool CWindowControl::IsMaxButton() const { return _isMaxButton; }

bool CWindowControl::IsMinButton() const { return _isMinButton; }

std::wstring CWindowControl::GetTitleName() const { return _titleName; }

bool CWindowControl::IsTitleBar() const { return _isTitlebar; }

bool CWindowControl::IsSizable() const { return _isSizable; }
...
int CWindowControl::GetActiveEvent() const { return _activeEvent; }

int CWindowControl::GetCloseEvent() const { return _closeEvent; }

int CWindowControl::GetSizeEvent() const { return _sizeEvent; }

int CWindowControl::GetKeyDownEvent() const { return _keyDownEvent; }

int CWindowControl::GetKeyUpEvent() const { return _keyUpEvent; }

COLORREF CWindowControl::GetBackgroundColor() { return _backgroundColor; }

HBRUSH CWindowControl::GetBackgroundBrush() { return _backBrush; }

CMenu *CWindowControl::GetMenu() { return _menu; }

CToolbarControl *CWindowControl::GetToolbar() { return _toolbar; }
...
void CWindowControl::SetY(const int y) {
  ...
  if (_hWnd != nullptr) {
    ...
    SetRect(&rect, GetX(), _position.y, GetX() + GetWidth(),
    ...
    int diffX = 0;
    int diffY = 0;
    SetRect(&rect, rect.left + diffX, rect.top + diffY, rect.right + diffX,
            rect.bottom + diffY);

    SetWindowPos(_hWnd, nullptr, rect.left, rect.top, rect.right - rect.left,
                 rect.bottom - rect.top, SWP_NOSIZE || SWP_NOZORDER);
  }
}
...
void CWindowControl::SetX(const int x) {
  ...
  if (_hWnd != nullptr) {
    ...
    SetRect(&rect, _position.x, GetY(), _position.x + GetWidth(),
            GetY() + GetHeight());
    if (_menu) {
      AdjustWindowRect(&rect, _style, TRUE);
    } else {
...
void CWindowControl::SetIcon(std::wstring iconFilePath) {
  if (!iconFilePath.empty()) {
    _icon = static_cast<HICON>(
        LoadImage(nullptr, iconFilePath.c_str(), IMAGE_ICON, 0, 0,
...
void CWindowControl::AddLayout(CLayoutControl *layout, bool isShow) {
  ...
  if (layout->GetWidth() == 0 && layout->GetHeight() == 0) {
    ...
    SetRect(&rect, layout->GetX(), layout->GetY(),
...
bool CWindowControl::Create() {
  ...
  _hWnd = CreateWindowEx(
      WS_EX_DLGMODALFRAME, L"jojo_form", _titleName.c_str(), _style, rect.left,
      rect.top, rect.right - rect.left, rect.bottom - rect.top, parentHWnd,
      nullptr, CControlManager::GetInstance().GetHInstance(), (LPVOID)this);
...
void CWindowControl::Close() const { PostMessage(_hWnd, WM_CLOSE, 0, 0); }
...
int CWindowControl::ShowModalWindow() {
  ...
  SetWindowPos(_parentControl->GetHWnd(), HWND_NOTOPMOST, 0, 0, 0, 0,
