...
static int GetPacketAudioQueue(AudioState* audioState, AudioPacketQueue* queue,
                               AVPacket* packet) {
  int result;
...
static int ResamplingAudio(AVCodecContext* audio_decode_ctx,
  ...
  if (in_channel_layout <= 0) {
    Console::OutputConsoles(L"in_channel_layout error\n");
    return -1;
  }
  ...
  if (in_nb_samples <= 0) {
    Console::OutputConsoles(L"in_nb_samples error\n");
    return -1;
  }

  av_opt_set_int(swr_ctx, "in_channel_layout", in_channel_layout, 0);
  av_opt_set_int(swr_ctx, "in_sample_rate", audio_decode_ctx->sample_rate, 0);
  av_opt_set_sample_fmt(swr_ctx, "in_sample_fmt", audio_decode_ctx->sample_fmt,
                        0);
  ...
  if ((ret = swr_init(swr_ctx)) < 0) {
    Console::OutputConsoles(
        L"Failed to initialize the resampling context\n");
    return -1;
  }
  ...
  if (max_out_nb_samples <= 0) {
    Console::OutputConsoles(L"av_rescale_rnd error\n");
    return -1;
  }
  ...
  if (ret < 0) {
    Console::OutputConsoles(
        L"av_samples_alloc_array_and_samples error\n");
    return -1;
  }
  ...
  if (swr_ctx) {
    ...
    if (ret < 0) {
      Console::OutputConsoles(L"swr_convert_error\n");
      return -1;
    }
    ...
    if (resampled_data_size < 0) {
      Console::OutputConsoles(L"av_samples_get_buffer_size error\n");
      return -1;
    }
  } else {
    Console::OutputConsoles(L"swr_ctx null error\n");
    return -1;
  }
...
int DecodeMusicFrame(AudioState* audioState, uint8_t* audioBuffer,
                     int bufferSize) {
  int dataSize = 0;
  ...
  for (;;) {
    while (audioState->audioPacketSize > 0) {
      int packetError =
          avcodec_send_packet(audioState->audioCodecContext, packet);
      ...
      dataSize = 0;
      if (avcodec_receive_frame(audioState->audioCodecContext,
                                &audioState->audioFrame) == 0) {
        dataSize = ResamplingAudio(
            audioState->audioCodecContext, &audioState->audioFrame,
            AV_SAMPLE_FMT_S16, audioState->audioFrame.channels,
            audioState->audioFrame.sample_rate, audioBuffer);
      }
...
void MusicAudioCallback(void* userdata, Uint8* stream, int len) {
  ...
  while (len > 0) {
    if (audioState->audioBufferIndex >= audioState->audioBufferSize) {
      ...
      int audioSize = DecodeMusicFrame(audioState, audioState->audioBuffer,
                                       sizeof(audioState->audioBuffer));
      if (audioSize < 0) {
    ...
    int computedLen =
        audioState->audioBufferSize - audioState->audioBufferIndex;
    if (computedLen > len) {
      computedLen = len;
    }
    memcpy(stream,
           (uint8_t*)audioState->audioBuffer + audioState->audioBufferIndex,
           computedLen);
    len -= computedLen;
    stream += computedLen;
...
CAudioPlayerControl::~CAudioPlayerControl() { Destroy(); }

bool CAudioPlayerControl::IsPlaying() { return _state.playing; }

int CAudioPlayerControl::Read(void* opaque, unsigned char* buf, int buf_size) {
  MemoryStream* stream = static_cast<MemoryStream*>(opaque);
...
int64_t CAudioPlayerControl::Seek(void* opaque, int64_t offset, int whence) {
  ...
  if (0x10000 == whence) return stream->Size();

  stream->seekg(offset, whence);
  return stream->tellg();
}

bool CAudioPlayerControl::LoadFromMe5File(std::wstring filePath, int groupIndex,
                                          int subIndex) {
  ...
  _inputStream = new MemoryStream(by, size);
  _state.formatContext = avformat_alloc_context();
  _state.formatContext->pb = avio_alloc_context(
      buffer, size, 0, _inputStream, &CAudioPlayerControl::Read, nullptr,
  ...
  if (error < 0) {
    Console::OutputConsoles(L"File open error");
    return false;
  }

  if (avformat_find_stream_info(_state.formatContext, nullptr) < 0) {
    Console::OutputConsoles(L"Cannot find stream information");
    return false;
  }
  ...
  if (audioCodecParameters == nullptr) {
    Console::OutputConsoles(L"Cannot find audio stream");
    return false;
  }
  ...
  if (audioCodec == nullptr) {
    Console::OutputConsoles(L"Cannot find audio decoder");
    return false;
  }
  ...
  if (avcodec_parameters_to_context(_state.audioCodecContext,
    ...
    Console::OutputConsoles(L"Failed to convert to context");
    return false;
  }

  if (avcodec_open2(_state.audioCodecContext, audioCodec, nullptr) < 0) {
    Console::OutputConsoles(L"Cannot open codec");
    return false;
  }
...
bool CAudioPlayerControl::LoadFromFile(std::wstring filePath) {
  ...
  int length = WideCharToMultiByte(CP_UTF8, 0, filePath.c_str(), -1, nullptr, 0,
                                   nullptr, nullptr);
  char* buffer = new char[length + 1];
  WideCharToMultiByte(CP_UTF8, 0, filePath.c_str(), -1, buffer, length, nullptr,
                      nullptr);
