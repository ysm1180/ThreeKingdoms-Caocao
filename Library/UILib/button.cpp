...
LRESULT CALLBACK CButtonControl::OnControlProc(HWND hWnd, UINT iMessage,
  ...
  switch (iMessage) {
    case WM_CREATE: {
      ...
      if (createEvent != LUA_NOREF) {
        LuaTinkerHelper::GetLuaTinker().Call(createEvent, button);
      }
      break;
    }
    ...
    case WM_LBUTTONUP: {
      ...
      if (mouseLButtonUpEvent != LUA_NOREF) {
        LuaTinkerHelper::GetLuaTinker().Call(mouseLButtonUpEvent, button,
    ...
    case WM_LBUTTONDOWN: {
      ...
      if (mouseLButtonDownEvent != LUA_NOREF) {
        LuaTinkerHelper::GetLuaTinker().Call(mouseLButtonDownEvent, button,
                                        (int)wParam, GET_X_LPARAM(lParam),
                                        GET_Y_LPARAM(lParam));
      }
    ...
    case WM_MOUSEMOVE: {
      ...
      if (mouseMoveEvent != LUA_NOREF) {
        LuaTinkerHelper::GetLuaTinker().Call(mouseMoveEvent, button, (int)wParam,
                                        GET_X_LPARAM(lParam),
                                        GET_Y_LPARAM(lParam));
      }
    ...
    case WM_MOUSEHOVER: {
      ...
      if (mouseHoverEvent != LUA_NOREF) {
        LuaTinkerHelper::GetLuaTinker().Call(mouseHoverEvent, button);
      }
    ...
    case WM_MOUSELEAVE: {
      ...
      if (mouseLeaveEvent != LUA_NOREF) {
        LuaTinkerHelper::GetLuaTinker().Call(mouseLeaveEvent, button);
      }
    ...
    case WM_DESTROY: {
      ...
      if (destroyEvent != LUA_NOREF) {
        LuaTinkerHelper::GetLuaTinker().Call(destroyEvent, button);
      }
      break;
    }
  }
  return CallWindowProc(CButtonControl::GetOriginalProc(), hWnd, iMessage,
                        wParam, lParam);
}
...
CButtonControl::CButtonControl() : _font(this) {
...
CTextFont* CButtonControl::GetFont() { return &_font; }

std::wstring CButtonControl::GetText() const { return _text; }

ButtonStateColor& CButtonControl::GetBackgroundColor() {
  return _backgroundColor;
}

ButtonStateColor& CButtonControl::GetBorderColor() { return _borderColor; }

ButtonStateColor& CButtonControl::GetTextColor() { return _textColor; }
...
int CButtonControl::GetBorderWidth() const { return _borderWidth; }
...
void CButtonControl::SetPushedBorderColor(COLORREF color) {
  if (_borderColor.pushed != color) _borderColor.pushed = color;
}
...
bool CButtonControl::Create() {
  if (_parentControl != nullptr) {
    _hWnd = CreateWindow(
        L"jojo_button", _text.c_str(), _style, _position.x, _position.y,
        _size.cx, _size.cy, _parentControl->GetHWnd(), (HMENU)this,
        CControlManager::GetInstance().GetHInstance(), (LPVOID)this);
    _font.ResetFont();
  }
...
WNDPROC CButtonControl::GetOriginalProc() { return s_originalProc; }
}  // namespace three_kingdoms